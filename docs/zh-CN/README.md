...
Node.js 20+。克隆、启动，剩下的一切都在 Web UI 里做：
...
打开 <http://127.0.0.1:8787/> 登录（默认密码 `admin123`）。从这里开始，
不需要碰任何配置文件：

- **渠道 tab**——粘贴 Key；同一渠道多 Key 自动轮换，401 只退役那一个
- **访问 tab**——给 `/v1/*` 客户端建网关 API Key
- **路由 tab**——排每条路由试模型的顺序
- **配额 tab**——日限额和用量历史
- **设置 tab**——服务、发现、调优，以及**允许局域网访问**开关（会列出
  确切可用地址，容器环境能识别）

界面跟随浏览器语言（内置 12 种语言）。
`./ctl.sh stop` 停止；`./ctl.sh restart`、`./ctl.sh status`、
`./ctl.sh docker [--dev]` 覆盖其余操作。详见[工作原理](HOW_IT_WORKS.md)。
...
一个 provider 需要多个 Key？`OPENROUTER_API_KEYS`（或
`OPENROUTER_API_KEY_KEYS`），逗号分隔填多个值；也可以在 Web UI 里添加
命名 Key——请求会在它们之间自动轮换。

加新渠道：在 Web UI 里只填名字和 base URL 添加，或在
`app/config/config.json` 里加一段。

## 局域网访问

设置 → **允许局域网访问**，会列出所有可达地址（`{lan-ip}`、
`127.0.0.1`、`localhost`、`{hostname}`）。暴露之前先做这两件事：
...
`app/config/config.json` 只放默认值，保持可合并。Web UI 里的一切改动都
写进 gitignored 的 `data/config.local.json`，启动时覆盖默认值（对象按
key 合并，数组整体替换）。完整优先级（从高到低）：显式进程环境变量 →
`data/config.local.json` → 首启 `.env` 种子 → `app/config/config.json`。
...
Web UI 是主要配置方式，`.env` 只是个便利的种子。`mkdir -p data &&
cp .env.example data/.env`，填上 Key（局域网再加
`FREE_ROUTER_HOST=0.0.0.0`），首启会被迁移进 `data/config.local.json`。
之后 `.env` 文件彻底忽略，UI 改的一定生效；显式进程环境变量永远最高。
